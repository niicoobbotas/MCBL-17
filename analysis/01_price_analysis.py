...
Part 1: NL Electricity Price Analysis
----------------------------------------------------------------------

Reads:
  - datasets/dataset7/european_wholesale_electricity_price_data_daily.csv
  - datasets/dataset7/Netherlands.csv  (real hourly, 2015-2025)
----------------------------------------------------------------------
Outputs:
  - plots/p01_nl_price_history.png
  - plots/p02_annual_mean_volatility.png
  - plots/p03_monthly_seasonality.png
  - plots/p04_dow_pattern.png
  - plots/p05_country_comparison.png
  - plots/p06_hourly_profile.png
  - results/nl_price_stats.csv
  - results/hourly_profile.csv
"""
import os
import numpy as np
import pandas as pd
...
# Plot 1: p01_nl_price_history.png
...
fig.savefig(os.path.join(PLOT_DIR, "p01_nl_price_history.png"))
plt.close(fig)

# ---- Plot 2: Annual mean & volatility -----------------------------------
# Plot 2: p02_annual_mean_volatility.png
...
ax2.grid(False)
ax1.set_title("Annual price level and volatility, NL (2023-2025)")
ax1.set_xticks(ann["year"])
fig.savefig(os.path.join(PLOT_DIR, "p02_annual_mean_volatility.png"))
plt.close(fig)

# ---- Plot 3: Monthly seasonality (post-crisis: 2023-2025) ---------------
# Plot 3: p03_monthly_seasonality.png
...
fig.savefig(os.path.join(PLOT_DIR, "p03_monthly_seasonality.png"))
plt.close(fig)

# ---- Plot 4: Day-of-week pattern ----------------------------------------
# Plot 4: p04_dow_pattern.png
...
fig.savefig(os.path.join(PLOT_DIR, "p04_dow_pattern.png"))
plt.close(fig)

# ---- Plot 5: Cross-country comparison, recent ---------------------------
# Plot 5: p05_country_comparison.png
...
ax.invert_yaxis()
fig.savefig(os.path.join(PLOT_DIR, "p05_country_comparison.png"))
plt.close(fig)

# ---- Real hourly diurnal profile (from Netherlands.csv) -----------------
# Use 2023-2025 recent period for the profile used in the algorithm
# Table: hourly_profile.csv with R: hour, eur_per_mwh, eur_per_kwh, std_mwh, median_mwh
...
ax.set_ylabel("EUR / MWh")
ax.set_title("Wholesale day-ahead hourly price profile, NL (2023-2025)")
ax.legend(loc="upper left", frameon=False)
fig.savefig(os.path.join(PLOT_DIR, "p06_hourly_profile.png"))
plt.close(fig)
