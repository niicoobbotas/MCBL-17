...
Part 4: Vehicle Fleet Database
----------------------------------------------------------------------

Data (all open / public):
  - CBS 85237NED  NL passenger cars by fuel type, 1 Jan each year
  - EAFO NL country profile  (cross-validation of national totals)
  - RVO Cijfers elektrisch vervoer  (model-level registrations)
  - EV-database.org  (vehicle specs: battery, range, efficiency)
  - CBS Bevolking gemeente 2024  (Eindhoven population 246,443)
----------------------------------------------------------------------
Outputs:
  - results/databases/04_fleet/fleet_database.json
  - results/databases/04_fleet/ev_models.csv
  - results/databases/04_fleet/nl_fleet_yearly.csv
  - results/databases/04_fleet/eindhoven_fleet_yearly.csv
  - results/databases/04_fleet/ownership_split.csv
"""
import os
import json
import pandas as pd
...
OUT_DIR = os.path.join(BASE, "..", "results", "databases", "04_fleet")
