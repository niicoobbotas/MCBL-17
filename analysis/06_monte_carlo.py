...
Part 6: Monte Carlo Simulation (v2 — real data)
----------------------------------------------------------------------
...
Reads:
  ...
  - results/databases/05_behavior/trip_length_distribution.csv
  - datasets/dataset6/eindhoven_zonal_load.csv
----------------------------------------------------------------------
Outputs:
  - plots/p21_cost_4_policies.png
  - plots/p22_savings_by_location.png
  - plots/p23_peak_hour_drop.png
  - plots/p24_tariff_compare_2024.png
  - plots/p25_fixed_vs_smart_scatter.png
  - results/databases/06_monte_carlo/v2_monte_carlo.csv
  - results/databases/06_monte_carlo/kpi_summary.csv
"""
import os
import json
import numpy as np
...
OUT_DIR  = os.path.join(DB, "06_monte_carlo")
...
trip_dist = pd.read_csv(os.path.join(DIR_05, "trip_length_distribution.csv"))

# Real hourly prices
hourly_real = pd.read_csv(os.path.join(DIR_02, "nl_hourly_real_2015_2025.csv"))
...
def schedule_smart(E_req, P_max, eta, window, p_kwh, L_norm, L_cap=0.6,
    ...
    x = np.zeros(window); remaining = E_req
    iterations = 0
    while remaining > 1e-6 and iterations < 20000:
        iterations += 1
...
mc.to_csv(os.path.join(OUT_DIR, "v2_monte_carlo.csv"), index=False)
...
fig.savefig(f"{PLOT_DIR}/p21_cost_4_policies.png")
plt.close(fig)
...
ax.set_title("Smart-charging savings by location type")
for i, n in enumerate(loc_summary["n"]):
    ax.text(i, -1.5, f"n={n}", ha="center", fontsize=9, color=GREY)
ax.legend(frameon=False, loc="upper right", fontsize=9)
fig.savefig(f"{PLOT_DIR}/p22_savings_by_location.png")
plt.close(fig)
...
ax.set_xlabel("Annual peak-hour charging (kWh/year), between 17-21h")
ax.set_ylabel("Number of drivers")
ax.set_title("Grid relief - peak-hour charging displaced by Smart scheduling")
ax.legend(loc="upper right", frameon=False)
fig.savefig(f"{PLOT_DIR}/p23_peak_hour_drop.png")
plt.close(fig)
...
ax.set_title("Eneco dynamic vs. fixed tariffs, 2024 (wholesale incl. tax + BTW)")
ax.legend(loc="upper right", frameon=False, fontsize=9)
ax.set_ylim(0, 0.5)
fig.savefig(f"{PLOT_DIR}/p24_tariff_compare_2024.png")
plt.close(fig)

# (e) Fixed vs Smart scatter — one dot per driver, diagonal = break-even
fig, ax = plt.subplots(figsize=(8, 7))
ax.scatter(mc["cost_fix"], mc["cost_sm"], alpha=0.35, s=18,
           c=mc["save_sm_vs_fix_pct"], cmap="RdYlGn", vmin=-5, vmax=40)
lim = max(mc["cost_fix"].max(), mc["cost_sm"].max()) * 1.05
ax.plot([0, lim], [0, lim], "--", color=GREY, lw=1.2, label="Break-even (smart = fixed)")
ax.set_xlabel("Annual cost - Fixed tariff (€/yr)")
ax.set_ylabel("Annual cost - Smart CACCS (€/yr)")
ax.set_title(f"Fixed tariff versus Smart scheduler with dynamic tariffs cost per driver\n"
             f"{len(mc)} Eindhoven drivers (2024 prices)\n"
             "Green = smart cheaper; red = fixed cheaper")
ax.legend(frameon=False)
sm = plt.cm.ScalarMappable(cmap="RdYlGn", norm=plt.Normalize(-5, 40))
fig.colorbar(sm, ax=ax, label="Saving vs fixed (%)")
fig.savefig(f"{PLOT_DIR}/p25_fixed_vs_smart_scatter.png")
plt.close(fig)
...
kpi = {
    "n_drivers":                  len(mc),
    "mean_cost_fix_EUR":          mc["cost_fix"].mean(),
    "mean_cost_unc_EUR":          mc["cost_unc"].mean(),
    "mean_cost_po_EUR":           mc["cost_po"].mean(),
    "mean_cost_sm_EUR":           mc["cost_sm"].mean(),
    "median_cost_sm_EUR":         mc["cost_sm"].median(),
    "mean_save_sm_vs_fix_pct":    mc["save_sm_vs_fix_pct"].mean(),
    "mean_save_sm_vs_unc_pct":    mc["save_sm_vs_unc_pct"].mean(),
    "share_drivers_save_vs_fix":  (mc["cost_sm"] < mc["cost_fix"]).mean(),
    "share_drivers_save_vs_unc":  (mc["cost_sm"] < mc["cost_unc"]).mean(),
    "mean_peak_drop_pct":         mc["peak_drop_pct"].mean(skipna=True),
    "mean_annual_km":             mc["annual_km"].mean(),
}
pd.Series(kpi).to_csv(os.path.join(OUT_DIR, "kpi_summary.csv"))
print("\nKPI summary:")
for k, v in kpi.items():
    print(f"  {k}: {v:.3f}")

print(f"\nPart 6 simulation complete. Plots in {PLOT_DIR}")
