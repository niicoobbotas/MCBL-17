...
Part 5: Charging Behavior & Tariff Database
----------------------------------------------------------------------

Reads:
  - ElaadNL Open Data Dashboard  (arrival distributions, connection times)
  - CBS ODiN 2023  (trip-length distribution by purpose)
  - Eneco / Essent published tariff sheets Jan-May 2026
  - datasets/oplaadpalen.csv  (public charger locations, optional)
----------------------------------------------------------------------
Outputs:
  - plots/p18_arrival_distributions.png
  - plots/p19_trip_length_distribution.png
  - plots/p20_home_charging_share.png
  - results/databases/05_behavior/arrival_distributions.csv
  - results/databases/05_behavior/connection_time_stats.csv
  - results/databases/05_behavior/trip_length_distribution.csv
  - results/databases/05_behavior/tariffs_eneco_vs_essent.csv
  - results/databases/05_behavior/ehv_home_charging_share.csv
"""
import os
import json
import numpy as np
...
OUT_DIR = os.path.join(BASE, "..", "results", "databases", "05_behavior")
...
ax.set_title("Arrival-time distribution of EV charging by location type,\n"
             "ElaadNL Open Data (2018–2020)")
ax.legend(loc="upper left", frameon=False)
fig.savefig(f"{PLOT_DIR}/p18_arrival_distributions.png")
plt.close(fig)
...
fig.savefig(f"{PLOT_DIR}/p19_trip_length_distribution.png")
plt.close(fig)
...
charger_file = os.path.join(BASE, "..", "datasets", "oplaadpalen.csv")
...
ax.barh(sorted_pc["pc4"].astype(str), sorted_pc["home_charging_share_pct"], color=colors, height=0.75)
...
fig.savefig(f"{PLOT_DIR}/p20_home_charging_share.png")
plt.close(fig)

print("\n Part 5 (behavior + tariffs + trips + home charging) complete.")
