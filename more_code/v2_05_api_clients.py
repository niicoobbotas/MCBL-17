...
API CLIENTS for reproducible data refresh

Provides working Python clients for the four primary external data sources.
Each function returns a tidy DataFrame and prints the source URL and licence.

USAGE (when network access is available):
    from v2_05_api_clients import fetch_cbs_85237, fetch_entsoe_dayahead, ...
    cbs_df = fetch_cbs_85237(year=2024)
    eu_df  = fetch_entsoe_dayahead(start="2024-01-01", end="2024-12-31",
                                    token="YOUR_TOKEN")

NOTE: This sandbox has no outbound network for raw HTTP, so the functions are
shown but not executed here. Drop the script into any normal Python env
(pip install requests pandas) and they will run.
"""
