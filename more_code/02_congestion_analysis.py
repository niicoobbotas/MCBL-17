...
fig.savefig(os.path.join(PLOT_DIR, "p7_eindhoven_hourly_load.png"))
plt.close(fig)
...
cb.set_label("MW")
fig.savefig(os.path.join(PLOT_DIR, "p8_zone_hour_heatmap.png"))
plt.close(fig)
...
ax.invert_yaxis()
fig.savefig(os.path.join(PLOT_DIR, "p9_tennet_headroom.png"))
plt.close(fig)
...
fig.savefig(os.path.join(PLOT_DIR, "p10_ehv_pc4_congestion.png"))
plt.close(fig)
...
ax1.plot(hp["hour"], hp["eur_per_mwh"] / hp["eur_per_mwh"].max(),
         "s--", color=WARN, lw=2.0, label="Price (normalised, real hourly)")
...
fig.savefig(os.path.join(PLOT_DIR, "p11_load_vs_price.png"))
plt.close(fig)
